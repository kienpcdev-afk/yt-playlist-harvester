...
4. [Cài đặt & chạy project](#4-cài-đặt--chạy-project)
   - [Cách 1 — Chạy bằng Node.js](#cách-1--chạy-bằng-nodejs-phát-triển)
   - [Cách 2 — Build & chạy file `.exe`](#cách-2--build--chạy-file-exe-phân-phối)
...
├── app-paths.js          # Đường dẫn dữ liệu khi chạy .exe
...
├── yt-playlist-harvester.exe   # ← file build (sau npm run build:exe)
...
Có **hai cách** chạy tool trên Windows:

| Cách | Khi nào dùng | Cần Node.js lúc chạy? |
|---|---|---|
| [Cách 1 — Node.js](#cách-1--chạy-bằng-nodejs-phát-triển) | Phát triển, sửa code | Có |
| [Cách 2 — file `.exe`](#cách-2--build--chạy-file-exe-phân-phối) | Phân phối, click đúp chạy | Không (chỉ cần khi build) |

---

### Cách 1 — Chạy bằng Node.js (phát triển)
...
### Cách 2 — Build & chạy file `.exe` (phân phối)

Đóng gói toàn bộ server + giao diện web vào **một file** `yt-playlist-harvester.exe`. Người dùng cuối **không cần cài Node.js** để chạy — chỉ cần `ffmpeg`, `yt-dlp.exe` và file `.exe` cùng thư mục.

#### Yêu cầu để build (máy developer)

| Thành phần | Ghi chú |
|---|---|
| Node.js 18+ | Dùng khi build; `pkg` hỗ trợ tốt nhất `node18-win-x64` |
| npm | Đi kèm Node.js |
| Source project | Đã `npm install` ít nhất một lần |

#### Bước 1 — Cài `pkg` (một lần)

Cài toàn cục (tùy chọn):

```powershell
npm install -g pkg
```

Hoặc dùng `pkg` qua `npx` / `devDependency` sẵn có trong project (không cần cài global).

Nếu `npm install` báo lỗi SSL certificate:
...
#### Bước 2 — Chuẩn bị thư mục project

Đảm bảo đã có trong thư mục gốc project:
...
File bắt buộc cạnh nơi build:

- `yt-dlp.exe` — [mục 2.2](#22-tải-yt-dlpexe)
- `.env` — tùy chọn, copy từ `.env.example`
...
Lệnh tương đương (build thẳng ra thư mục gốc project):

```powershell
npx pkg . --targets node18-win-x64 --output yt-playlist-harvester.exe
```

Sau khi build thành công, file nằm tại:

```
D:\Tool\yt-playlist-harvester\yt-playlist-harvester.exe
```

> **Lưu ý:** `yt-dlp.exe` **không** được gói vào trong `.exe` — luôn đặt **cùng thư mục** với file thực thi. `cookies.txt`, `downloads/` cũng được tạo/đọc **cạnh file `.exe`**.

#### Bước 4 — Chạy file `.exe`

**Click đúp** `yt-playlist-harvester.exe` — server khởi động và trình duyệt tự mở `http://localhost:3000`.
...
Giữ cửa sổ Terminal mở trong khi dùng. Đóng cửa sổ = dừng server.

Khi thành công, terminal hiển thị (ví dụ):

```
Server đang chạy tại http://localhost:3000
Thư mục dữ liệu: D:\Tool\yt-playlist-harvester
Thư mục tải: D:\Tool\yt-playlist-harvester\downloads
yt-dlp: D:\Tool\yt-playlist-harvester\yt-dlp.exe
```

#### Cấu trúc thư mục khi chạy `.exe`

```
D:\Tool\yt-playlist-harvester\
├── yt-playlist-harvester.exe   ← file build
├── yt-dlp.exe                  ← bắt buộc
├── .env                        ← tùy chọn
├── cookies.txt                 ← tự tạo (Extension / setup)
├── downloads\                  ← tự tạo khi tải video
└── chrome-extension\           ← cài vào Chrome nếu cần cookie (mục 6)
```
...
#### Tùy chọn môi trường khi chạy `.exe`

| Biến | Ý nghĩa |
|---|---|
| `OPEN_BROWSER=0` | Không tự mở trình duyệt khi khởi động |
| `OPEN_BROWSER=1` | Luôn mở trình duyệt (kể cả khi chạy `node`) |
| `PORT=3001` | Đổi cổng (đặt trong `.env` cạnh `.exe`) |
...
**Phương án A — Chạy bằng Node.js (source đầy đủ)**
...
**Phương án B — Chỉ dùng file `.exe` (không cần Node.js)**

Gửi kèm (cùng một thư mục):

| File / thư mục | Bắt buộc |
|---|---|
| `yt-playlist-harvester.exe` | Có |
| `yt-dlp.exe` | Có |
| `ffmpeg` (cài winget hoặc PATH) | Có |
| `.env.example` → đổi tên `.env` | Tùy chọn |
| `chrome-extension/` | Nếu cần cookie playlist >100 |
...
Xem chi tiết build: [Cách 2 — Build & chạy file `.exe`](#cách-2--build--chạy-file-exe-phân-phối).
...
### Build & chạy file `.exe` (phân phối)

```powershell
# Máy build (cần Node.js)
cd D:\Tool\yt-playlist-harvester
npm install
npm run build:exe
# → tạo yt-playlist-harvester.exe trong thư mục project

# Máy người dùng (không cần Node.js — cần ffmpeg + yt-dlp.exe cùng thư mục)
cd D:\Tool\yt-playlist-harvester
.\yt-playlist-harvester.exe
